...
async def register(data: dict):
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
        return {"success": True}
    except: return {"success": False, "msg": "اليوزر مستخدم مسبقاً"}
    finally: conn.close()
...
async def login(data: dict):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT username_key, player_name FROM users WHERE username_key = %s AND password_key = %s",
...
<head>
    ...
    <style>
        ...
        .card { background: var(--card); padding: 30px; border-radius: 30px; box-shadow: 0 15px 35px rgba(0,0,0,0.6); border: 2px solid #3c339e; }
        ...
        .q-badge { background: var(--error); padding: 5px 15px; border-radius: 10px; font-size: 14px; margin-bottom: 10px; display: inline-block; }
...
<body>
    ...
    <div id="sidebar" class="sidebar">
        ...
        <p id="user-display" style="font-weight:bold;">زائر</p>
        <button style="background:var(--error)" onclick="logout()">تسجيل الخروج</button>
        <button style="background:#636e72" onclick="toggleSidebar()">إغلاق</button>
    </div>
    ...
    <script>
        ...
        function showAuth() {
            document.getElementById('main-ui').innerHTML = `
                <div class="card">
                    ...
                    <input id="r_nick" placeholder="الاسم الذي يظهر للجميع">
        ...
        function showMenu() {
            document.getElementById('main-ui').innerHTML = `
                <div class="card">
                    ...
                    <button style="background:#e056fd" onclick="showSetup()">🏠 أوفلاين (مجلس)</button>
                </div>`;
        }
        ...
        function showSetup() {
            document.getElementById('main-ui').innerHTML = `
                <div class="card">
                    <h2>إعداد الجلسة</h2>
                    <input type="number" id="p_count" value="3" min="3" onchange="genInputs()">
                    <div id="p_inputs"></div>
        ...
        function genInputs() {
            let n = Math.max(3, parseInt(p_count.value)), h = '';
            for(let i=1; i<=n; i++) h += `<input class="pn" placeholder="اللاعب ${i}" value="لاعب ${i}">`;
            document.getElementById('p_inputs').innerHTML = h;
        }

        async function start() {
            const players = Array.from(document.querySelectorAll('.pn')).map(i => i.value);
            ...
            game = await res.json(); game.players = players; game.curr = 0; showRole();
        }
        ...
        function showRole() {
            ...
            document.getElementById('main-ui').innerHTML = `
                <div class="card">
                    ...
                    <button onclick="document.getElementById('box').classList.remove('hidden'); this.style.display='none'; document.getElementById('bnxt').style.display='block'">اكشف</button>
        ...
        function showPhase1() {
            let qIdx = 0;
            function render() {
                if(qIdx >= game.q_seq.length) { showPhase2(game.players[0]); return; }
                const q = game.q_seq[qIdx];
                document.getElementById('main-ui').innerHTML = `
                    <div class="card"><span class="q-badge">المرحلة الإجبارية</span>
                        <div style="font-size:24px; margin:30px 0;"><b style="color:#a29bfe">${q.f}</b> ➜ يسأل ➜ <b style="color:#ff7675">${q.t}</b></div>
                        <button onclick="qIdx++; render()">التالي</button>
                        <button style="background:#636e72" onclick="startVoting()">بدء التصويت</button>
                    </div>`;
            }
            render();
        }
        ...
        function showPhase2(asker, last = "") {
            document.getElementById('main-ui').innerHTML = `
                <div class="card"><span class="q-badge" style="background:var(--primary)">مرحلة الاختيار الحر</span>
                    ...
                    <p style="font-size:14px">ممنوع تسأل اللي سألك (${last || '---'})</p>
                    <div id="plist"></div>
            ...
            setTimeout(() => {
                ...
                document.getElementById('main-ui').innerHTML = `
                    <div class="card">
                        ...
                        <h2 style="color:var(--error); font-size:40px; animation:rotate 0.2s 3">${spy}</h2>
